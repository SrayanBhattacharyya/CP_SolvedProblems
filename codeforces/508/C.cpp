...
int main(){
    ...
    ll a[NS], k = 0, ans = 0;
    bool sburn[NS];
    memset(sburn, false, sizeof(sburn));
    rep(i, m) cin >> a[i];
    if (r > t) {cout << -1 << "\n"; return 0;}
    vll candels;
    for (ll i = 1; i <= 300 && k < m; i++){
        if(a[k] == i){
            ll cnt = 0;
            vll temp;
            rep(j, candels.size()) if(candels[j] >= a[k]){
                ++cnt;
                temp.emplace_back(candels[j]);
            }
            candels = temp;
            ans += r - cnt;
            for(ll j =  i - 1; j >= i - (r - cnt); --j) candels.emplace_back(j + t);
            ++k;
        }
    }
    cout << ans << "\n";
