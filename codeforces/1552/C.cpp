...
int main(){
    ...
    while (t--){
        ...
        vll f;
        for (ll i = 1; i <= 2 * n; ++i)
            if (!vst[i]) f.push_back(i);
        for (ll i = 0, j = (f.size()) / 2; j < (f.size()); ++i, ++j)
            c.emplace_back(f[i], f[j]);
        ll cnt = 0;
        st(c);
        for (ll i = 0; i < n - 1; ++i) for (ll j = i + 1; j < n; ++j) cnt += c[i].second < c[j].second && c[i].second > c[j].first;
        cout << cnt << '\n';
    }    
    return 0;
}