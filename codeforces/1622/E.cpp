...
ll solvefor(vs s, vll v,ll k){
    string g;
    ll n = v.size();
    ll m = s[0].size();
    vll a, b;
    rep(i, n){
        if(k & (1LL << i)) a.emplace_back(i);
        else b.emplace_back(i);
    }
    vpll cnt(m, make_pair(0,0));
    for (auto x : a){
        rep(i, m){
            if(s[x][i] == '1') cnt[i].first++;
        }
    }
    for (auto x : b){
        rep(i, m){
    ...
    st(cnt);
    rep_a(i, 1, m + 1) p[cnt[i - 1].second] = i;
    ll ans = 0;
    rep(i, n){
        ll temp = 0;
        rep(j, m){
...
int main(){
    ...
    while (t--){
        ...
        pair <ll, vll> ans;
        ans.first = -1;
        ans.second.assign(m, 0);
        rep(i, 1LL << n){
            ll cur = solvefor(s, v, i);
            if (cur > ans.first){
                ans.first = cur;
                rep(j, m) ans.second[j] = p[j];
            }
        }
