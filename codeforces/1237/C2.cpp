...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        vector <pair <array<ll, 3>, ll>> points(n);
        rep(i, n){
            cin >> points[i].first[0] >> points[i].first[1] >> points[i].first[2];
		    points[i].second = i + 1;
        }
        sort(points.begin(), points.end());
        auto solve2 = [](auto points) {
            auto solve1 = [](auto pts) {
                ll n = (pts.size());
                for (ll i = 0; i + 1 < (pts.size()); i += 2) {
                    cout << pts[i].second << ' ' << pts[i + 1].second << '\n';
                }
                vll ex;
                if (pts.size() % 2 == 1) ex.push_back(pts.back().second);
                return ex;
            };
            ll n = (points.size());
            vector <pair<array<ll, 3>, ll>> cur;
            vll rest;
            rep(i, n){
                cur.push_back(points[i]);
                if (i + 1 == n || points[i].first[1] != points[i + 1].first[1]) {
                    auto res = solve1(cur);
                    assert(0 <= res.size() && res.size() <= 1);
                    for (auto it : res) rest.push_back(it);
                    cur.clear();
                }
            }
            for (ll i = 0; i + 1 < (rest.size()); i += 2) {
                cout << rest[i] << ' ' << rest[i + 1] << '\n';
            }
            vll ex;
            if (rest.size() % 2 == 1) ex.push_back(rest.back());
            return ex;
        };
        vector <pair<array <ll, 3>, ll>> cur;
        vll rest;
        rep(i, n){
            cur.push_back(points[i]);
            if (i + 1 == n || points[i].first[0] != points[i + 1].first[0]) {
                auto res = solve2(cur);
                assert(0 <= res.size() && res.size() <= 1);
                for (auto it : res) rest.push_back(it);
                cur.clear();
        ...
        assert(rest.size() % 2 == 0);
        for (ll i = 0; i < (rest.size()); i += 2) {
            cout << rest[i] << ' ' << rest[i + 1] << '\n';
        }
    }    
    return 0;
