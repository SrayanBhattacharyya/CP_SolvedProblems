...
char solve(vector <array<ll, 4>> &v, string &s, ll k){
    char ch = '-';
    if (k < 0) return ch;
    else if (k < s.size()) return s[k];
    else rep(i, v.size()) if (v[i][0] < k + 1 && v[i][1] > k - 1) return solve(v, s, v[i][2] + k - v[i][0]);
    return ch;
}
...
int main(){
    ...
    while (t--){
        ...
        while(c--){
            ll l, r; cin >> l >> r;
            a.push_back({idx, idx + (r - 1) - (l - 1), l - 1, r - 1});
            idx += (r - l + 1);
        }
        while (q--){
            ll x; cin >> x;
