...
ll tsuffix[200005], jsuffix[200005];
vll jk, tr;
...
int main(){
    ...
    while (t--){
        ...
        for (ll i = 0; i < n; i++){
            ll ip; cin >> ip;
            if (ip > m) tr.emplace_back(ip);
            else jk.emplace_back(ip);
        }
        dst(tr); dst(jk);
        rep_a(i, 1, tr.size() + 1) tsuffix[i] = tsuffix[i - 1] + tr[i - 1];
        rep_a(i, 1, jk.size() + 1) jsuffix[i] = jsuffix[i - 1] + jk[i - 1];
        ll ans1 = 0;
        rep(i, tr.size() + 1){
            ll ans = tsuffix[i];
            ll rd; 
            ...
            if (rd < 0) continue;
            rd = min(rd, (ll) jk.size()); ans += jsuffix[rd];
            ans1 = max(ans1, ans);
        } 
        cout << ans1 << "\n";
    }    
