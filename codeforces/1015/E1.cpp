...
int main(){
    ...
    while (t--){
        ll n, m;
        cin >> n >> m;
        rep_a(i, 1, n + 1)rep_a(j, 1, m + 1)    cin >> mat[i][j];
        rep_a(i, 1, n + 1)rep_a(j, 1, m + 1) if(mat[i][j] == '*'){
            int len = 0;
            int lo = j - 1, hi = j + 1;
            ...
            if(ans > 0){
                ...
                ok[i][j] = 1;
                int id = i - 1, loop = ans;
                while(id >= 1 && (loop-- > 0)) ok[id][j] = 1, id--;
                id = i + 1, loop = ans;
                while(id <= n && (loop-- > 0)) ok[id][j] = 1, id++;
                id = j - 1, loop = ans;
                while(id >= 1 && (loop-- > 0)) ok[i][id] = 1, id--;
                id = j + 1, loop = ans;
                while(id <= m && (loop-- > 0)) ok[i][id] = 1, id++;
            }
        }
        rep_a(i, 1, n + 1) rep_a(j, 1, m + 1) if(mat[i][j] == '*' && ok[i][j] == 0){
            cout << -1 << '\n';
            return 0;
        }
        cout << answer.size() << '\n';
        for(auto &it : answer) cout << it[0] << " " << it[1] << " " << it[2] << '\n';
    }    
    return 0;
}