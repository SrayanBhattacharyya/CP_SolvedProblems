...
// int repl(string s, string t, set <string> st){
//     string stemp;
//     rep(i, s.length() + 1){
//         if (i == s.length()) break;
//         if (s[i] == 'a'){
//             stemp = (s.replace(i, i, t));
//             st.insert(s.replace(i, i, t));
//         }
//         repl(stemp, t, st);
//     }
//     return st.size();
...
signed main(){
    ...
    while (t--){
        ...
        int cnta = 0;
        rep(i, m) if(t[i] == 'a') cnta++;
        if(cnta == 0) cout << pwr(2, n) << "\n";
        else if (cnta == 1 and m == 1) cout << 1 << "\n";
        else cout << -1 << "\n";
    }    
    return 0;
}