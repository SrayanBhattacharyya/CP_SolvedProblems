...
int main(){
    ...
    while (t--){
        ...
        cin >> n >> p; cin >> s;
        bool fl = true;
        for (ll i = 0; i < n / 2; i++) if (s[i] != s[n - i - 1]) fl = false;
        if (fl) {cout << 0 << "\n"; return 0;}
        ll l, r, l1, r1;
        for (ll i = 0; i < n / 2; i++) if (s[i] != s[n - i - 1]) {l = i; break;}
        for (ll i = n / 2 - 1; i >= 0; i--) if (s[i] != s[n - i - 1]) {r = i; break;}
        for (ll i = (n + 1) / 2; i < n; i++) if (s[i] != s[n - i - 1]) {l1 = i; break;}
        for (ll i = n - 1; i >= (n + 1) / 2; i--) if (s[i] != s[n - i - 1])  {r1 = i; break;}
        ll ans = min(r - l + min(abs(p - 1 - r), abs(p - 1 - l)), r1 - l1 + min(abs(p - 1 - r1), abs(p - 1 - l1)));
        rep_a(i, l, r + 1) ans += min(min(s[i] - 'a' + 'z' - s[n - i - 1] + 1, abs(s[i] - s[n - i - 1])), s[n - i - 1] - 'a' + 'z' - s[i] + 1);
        cout << ans << "\n";
    }    
    return 0;
}