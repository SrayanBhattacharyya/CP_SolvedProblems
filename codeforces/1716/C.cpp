...
int main(){
    ...
    while (t--){
        ll n;
        cin >> n;
        vvll values(2, vll(n, 0)), dp(2, vll(n + 1,0));
        rep(i, 2) rep(j, n) cin >> values[i][j];
        dp[0][n - 1] = values[1][n - 1];
        dp[1][n - 1] = values[0][n - 1];
        rev(i, n - 2){
            dp[0][i] = max({dp[0][i+1]-1, values[1][i] - 2*(n-i-1), values[0][i+1]});
            dp[1][i] = max({dp[1][i+1]-1, values[0][i] - 2*(n-i-1), values[1][i+1]});
        }
        ll ans = LLMAX, curtime = -1;
        values[0][0] = -1;
        rep(i, n){
            ll r = i % 2, c = i;
            curtime = max(curtime + 1, values[r][c] + 1);
            ll extra = dp[r][c];
            ans = min(ans, 2 * (n - i) - 1 + max(extra, curtime));
            r ^= 1;
            curtime = max(curtime + 1, values[r][c] + 1);
        }
        cout << ans << '\n';
