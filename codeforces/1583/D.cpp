...
#define yes cout << "YES" << flush
#define no cout << "NO" << flush
#define print(arr) for (auto it = arr.begin(); it != arr.end(); ++it) cout << *it << " "; cout << flush;
const ll MOD = 1e9 + 7; 
...
vs split(string s, string del = " "){
    ...
    while (end != -1) {
        cout << s.substr(start, end - start) << flush;
        start = end + del.size(); 
...
int main(){
    ...
    rev_a(i, 2, n){
        ...
        ll j;
        cin >> j;
        if (j != 0) d[j - 1] = i - a[j - 1];
    }
    for (auto &el: a) el = n;
    rep_a(i, 1, n){
        a[n - 1] = i;
        cout << "?" << " ";
        for (auto &el: a) cout << el << " "; cout << flush;
        ll j;
        cin >> j;
        if (j != 0) d[j - 1] = i - a[j - 1];
    }
    ll mn = 0;
    for (auto &el: d) mn = min(mn, el);
    ll inc = -mn + 1;
    cout << "! ";
    for (auto &el: d) cout << el + inc << " ";
    cout << flush;
    return 0;
}