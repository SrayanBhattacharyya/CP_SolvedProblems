...
int main(){
    ...
    while (tc--){
        ...
        pll mx = { -1, -1 }, mn = { -1, -1 };
        rep(i, n) {
            if (mx.X == -1 || a[i] > a[mx.X]) mx = { i, mx.X };
            else if (mx.Y == -1 || a[i] > a[mx.Y]) mx = { mx.X, i };
        }
        for (ll i = n-1; i >= 0; i--) {
            if (mn.X == -1 || a[i] < a[mn.X]) mn = { i, mn.X };
            else if (mn.Y == -1 || a[i] < a[mn.Y]) mn = { mn.X, i };
        }
        cout << a[mx.X] - a[mn.X] + a[mx.Y] - a[mn.Y] << '\n';
    }    
    return 0;
}