...
ll pwr(ll x,ll k) {ll res = 1; while(k--){res*=x;} return res;}
...
vll lpf(NS);
void sieve(){
    rep_a(i, 2, NS) lpf[i] = i;
    rep_a(i, 2, NS) if(lpf[i] == i) for(ll j=i*i;j<NS;j+=i) if(lpf[j] == j) lpf[j] = i;
}
    ...
    while (t--){
        ...
        rep(i, n){
            ...
            while (cur != 1){
                ...
                cnt %= k;
                if (cnt) p.push_back({x, cnt});
            }
            ll curval = 1, reqval = 1;
            for (auto x : p){
                curval *= pwr(x.first,x.second);;
                reqval *= pwr(x.first,k-x.second);
            }
            ans += Cnt[reqval];
            Cnt[curval]++;
        }
