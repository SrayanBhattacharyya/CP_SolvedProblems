...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        vpll a(n); vll ans(n);
        vll dp1(n, LLMAX),dp2(n, LLMAX),dp3(n, LLMAX);
        rep(i, n){
            cin >> a[i].first;
            a[i].second = i;
        }
        sort(all(a));
        dp1[0] = 0;
        dp2[1] = a[1].first - a[0].first;
        rep_a(i, 2, n) {
            dp1[i] = dp3[i-1];
            dp2[i] = dp1[i-1]+a[i].first-a[i-1].first;
            dp3[i] = min(dp3[i-1],dp2[i-1])+a[i].first-a[i-1].first;
        }
        ll t = 1,s = 3;
        rev(i, n - 1) {
            ll j = a[i].second;
            ans[j] = t;
            if (s == 3) {
                if (dp2[i - 1] + a[i].first - a[i - 1].first == dp3[i]) s = 2;
            } 
            else if (s == 2) s = 1;
            else {
                s = 3;
