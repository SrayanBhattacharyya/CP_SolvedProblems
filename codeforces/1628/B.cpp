...
void solve(){
    ...
    bool fl = false;
    rep(i, n){
        if (s[i].size() == 1) {fl = true; break;}
        if (s[i].front() == s[i].back()) {fl = true; break;}
    }
    if (fl) {yes; return;}
    vector <vvll> v(4, vvll (26, vll (26,0)));
    rev(i, n - 1){
        ll a = s[i][0]-'a';
        ll b = s[i][1]-'a';
        if (s[i].size() == 2){
            if(v[2][b][a] || v[3][b][a]) {fl = true; break;}
        }
        else {if(v[2][b][a]) {fl = true; break;}}
        a = s[i][s[i].size() - 1] - 'a';
        b = s[i][s[i].size() - 2] - 'a';
        if(s[i].size() == 2) v[2][b][a] = 1;
        else v[3][b][a] = 1;
    }
    if (fl) {yes; return;}
    set <string> st;
    ...
    rep(i, n) rep_a(j, i + 1, n){
        if(s[i].size() == s[j].size() && s[j].size() == 3){
            fl = true;
            rep(k, 3) if(s[i][k] != s[j][2 - k]) fl = false;
            if (fl) {yes; return;}
        }
    }
    no;
