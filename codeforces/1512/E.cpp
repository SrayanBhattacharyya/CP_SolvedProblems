...
int main(){
    ...
    while (t--){
        ...
        vll q, ans;
        ll mn = z * (z + 1) / 2;
        ll mx = n * (n + 1) / 2 - (n - z) * (n - z + 1) / 2;
        if(s < mn || s > mx) {cout << -1 << "\n"; continue;}
        rep_a(i, 1, z + 1) ans.emplace_back(i);
        ans.emplace_back(n + 1);
        rev(i, z - 1) while(mn < s && ans[i] < ans[i + 1] - 1) {ans[i]++; mn++;}
        rep_a(i, 1, ans[0]) q.emplace_back(i);
        rep(i, z) rep_a(j, ans[i] + 1, ans[i + 1]) q.emplace_back(j);
        rep_a(i, 1, l) {cout << q.back() << " "; q.pop_back();}
        rep(i, z) cout << ans[i] << " ";
        rep_a(i, r + 1, n + 1) {cout << q.back() << " "; q.pop_back();}
        cout << "\n";
    }    
    return 0;
}