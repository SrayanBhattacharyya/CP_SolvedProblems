...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        vpll a(n + 1); vvll events;
        rep_a(i, 1, n + 1){
            cin >> a[i].first >> a[i].second;
            events.push_back({a[i].first, i, 1});
            events.push_back({a[i].second, i, -1});
        }
        vpll b = a;
        sort(all1(b)); 
        ll l = b[1].first, r = b[1].second;
        map <ll,ll> mp; mp[l] = 0; ll ans = 1;
        rep_a(i, 2, n + 1){
            if(b[i].first > r){
                ans++;
                l = b[i].first;
            ...
            else r = max(r, b[i].second);
        }
        vll ones(n + 1,0);
        rep_a(i, 1, n + 1) if(mp.find(a[i].first) != mp.end()) mp[a[i].first]++;
        rep_a(i, 1, n + 1) if(mp[a[i].first] == 1) ones[i]--;
        sll st; sort(all(events));
        rep(i, events.size() - 1){
            ll j = i;
            vll open, close;
            while(j < events.size() and events[j][0] == events[i][0]){
                if(events[j][2] == 1) open.emplace_back(events[j][1]);
                else close.emplace_back(events[j][1]);
                j++;
            }
            if(st.size() == 1 and open.size() >0) ones[*st.begin()]++;
            for(auto x : open) st.insert(x);
            for(auto x : close) st.erase(x);
            i = j - 1;
        }
        ans += (*max_element(all1(ones)));
        cout << ans << "\n";
    }    
    return 0;
}