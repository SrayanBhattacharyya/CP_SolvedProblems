...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        vpll prev; ll pre = 0;
        vll a(n); sll s; map <ll, ll> mp;
        rep(i, n) cin >> a[i];
        st(a); prev.push_back({0, n - 1});
        rev(i, 31){
            ll flag1 = 0;
            vpll ne, ne1;
            for (auto it : prev){
                ll flag = 0, j;
                ll c[2] = {0};
                for(j = it.first; j <= it.second; j++) c[(a[j] & (1 << i)) > 0]++;
                if(c[0]==0 or c[1]==0){
                    ne.push_back({it.first, it.second});
                    ne1.push_back({it.first, it.second});
                    flag1=1;
                }
                else{
                    ne1.push_back({it.first, it.first + c[0] - 1});
                    ne1.push_back({it.first + c[0], it.second});
                }
            }
            if(flag1){
                s.insert(i);
        ...
        ll k = 0;
        rep(i, 31) if(s.find(i) == s.end()) k += 1 << i;
        cout << k << "\n";
    }    
    return 0;
}