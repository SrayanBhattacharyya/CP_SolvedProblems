...
#define yes cout << "YES" << "\n"
#define no cout << "NO" << "\n"
#define print(aa2) for (auto it = aa2.begin(); it != aa2.end(); ++it) cout << *it << " "; cout << "\n";
const ll MOD = 1e9 + 7; 
...
int main(){
    ...
    while (t--){
        ...
        ll a1[NS], a2[NS + 2];
        rep(i, n) cin >> a1[i];
        memset(a2, -1, (NS + 2) * sizeof * a2);
        rep(i, n){
			k = 1;
			a2[a1[i] + 1] = max(a2[a1[i] + 1], INF);
			while (k <= k1) {
				b = a1[i] / k;
				k2 = min(b == 0 ? INF : a1[i] / b, k1);
				a = k2 == k1 ? -1 : a1[i] / (k2 + 1);
				a2[a + 1] = max(a2[a + 1], b);
				k = k2 + 1;
			}
		}
        for (a = 1; a <= NS; a++) a2[a] = max(a2[a], a2[a - 1]);
		ll ans = INF;
		for (a = 0; a <= NS; a++) ans = min(ans, a2[a] - a);
		cout << ans << "\n";
    }    
    return 0;
}