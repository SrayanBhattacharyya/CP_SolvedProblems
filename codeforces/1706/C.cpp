...
void solve(){
    ...
    if (n % 2){
        ll res = 0;
        for(ll  i = 2; i <= n-1; i += 2) res += max(0ll, max(a[i-1], a[i+1])  - a[i] + 1);
        cout << res << "\n";
        return;
    }
    rep_a(i, 2, n) b[i] = max(0ll, max(a[i+1], a[i-1]) - a[i] + 1);
    for(int i = 2; i < n ;i += 2) f[i] = f[i-2] + b[i];
    for(int i = 3; i < n; i += 2) s[i] = s[i-2] + b[i];
    ll res = LLMAX;
    rep_a(i, 2, n){
        if(i % 2 == 0) res = min(res, s[n - 1] - s[i - 1] + f[i - 2]);
        else res = min(res, f[n - 2] - f[i - 2] + s[i - 1]);
    }
    cout << res << "\n";
}
