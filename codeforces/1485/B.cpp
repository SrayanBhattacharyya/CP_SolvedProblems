...
int main(){
    ...
    while(t--){
        ...
        ll ans[n+1];
        ans[0] = 0;
        for(ll i  = 1; i <= n; i++) ans[i] = array[i] - array[i - 1] - 1;
        for(ll i = n; i > 0; i--) ans[i] += array[i + 1] - array[i] - 1;
        for(ll i = 2; i <= n; i++) ans[i] += ans[i - 1];
        ll l, r;
        ll finalAns = 0;
        while(q--)	{
            cin >> l >> r;
            finalAns = ans[r] - ans[l-1];
            finalAns -= array[l]-array[l-1]-1;
            finalAns -= array[r+1]-array[r]-1;
            finalAns += array[l]-1;
            finalAns += k-array[r];
            cout << finalAns << "\n";
    }
