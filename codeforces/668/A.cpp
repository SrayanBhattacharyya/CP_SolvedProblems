...
int main(){
    ...
    while (t--){
        ll n, m, qs;
        ll inp[NS][3]; ll ans[108][108]; ll q[NS];
        cin >> n >> m >> qs;
        rep(i, qs){
            cin >> q[i];
            cin >> inp[i][0];
            if(q[i] == 3) cin >> inp[i][1] >> inp[i][2];
        }
        rev(i, qs){
            if(q[i] == 3) ans[inp[i][0]][inp[i][1]] = inp[i][2];
            if(q[i] == 2){
                ll tmp = ans[n][inp[i][0]];
                for(ll j = n;j >= 1;j--) ans[j][inp[i][0]] = ans[j-1][inp[i][0]];
                ans[1][inp[i][0]] = tmp;
            }
            if(q[i] == 1){
                ll tmp = ans[inp[i][0]][m];
                for(ll j = m;j >= 1;j--) ans[inp[i][0]][j] = ans[inp[i][0]][j-1];
                ans[inp[i][0]][1] = tmp;
            }
        }
        rep_a(i, 1, n + 1){
