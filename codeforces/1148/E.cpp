...
int main(){
    ...
    st(s); st(t);
    vpll deltas;
    rep(i, n) deltas.emplace_back(t[i] - s[i].first, s[i].second);
    ll nextMinus = 0;
    while (deltas[nextMinus].first >= 0) {
        ++nextMinus;
        if (nextMinus == deltas.size()) break;
    }
    ...
    rep(i, n){
        ...
        if (deltas[i].first == 0) continue;
        while (deltas[i].first > 0) {
            if (nextMinus == deltas.size()) {
                no;
                return 0;
            }
            int kek = min(deltas[i].first, -deltas[nextMinus].first);
            ans.emplace_back(make_pair(deltas[i].second, deltas[nextMinus].second), kek);
            deltas[i].first -= kek;
            deltas[nextMinus].first += kek;
            if (deltas[nextMinus].first == 0) {
                while (deltas[nextMinus].first >= 0) {
                    ++nextMinus;
                    if (nextMinus == deltas.size()) break;
                }
    ...
    cout << "YES" << "\n" << ans.size() << "\n";
    rep(i, ans.size())  cout << ans[i].first.first + 1 << " " << ans[i].first.second + 1 << " " << ans[i].second << "\n";
    return 0;
}