...
int main(){
    ...
    rep(i, n){
        ...
        cin >> a >> b;
        pairs.push_back({a,b});
        if (a > b) dec.push_back(i);
        else if (a < b) inc.push_back(i);
    }
    if (dec.size() > inc.size()){
        sort(all(dec), [&](ll x, ll y) {return pairs[x].first < pairs[y].first;});
        cout << dec.size() << '\n';
        for (auto& x : dec)  cout << x + 1 << " ";
        cout << '\n';
    }
    else{
        sort(all(inc), [&](int x, int y){ return pairs[x].second > pairs[y].second;});
        cout << inc.size() << '\n';
