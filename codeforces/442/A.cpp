...
int main(){
    ...
    while (t--){
        ll n;
        cin >> n;
        for(ll i = 0; i < 5; i++) mp1[s1[i]] = i, mp2[s2[i]] = i;
        for(ll i = 0; i < n; i++){
            string s;
            cin >> s;
            F[i] = mp1[s[0]];
        ...
        ll ans = 10;
        for(ll mask1 = 0; mask1 < 32; mask1++){
            for(ll mask2 = 0; mask2 < 32; mask2++){
                bool valid = 1;
                for(ll i = 0; i < n; i++){
                    for(ll j = i + 1; j < n; j++){
                        if(!(F[i] == F[j] && G[i] == G[j])){
                            ll x = F[i] == F[j] ? 0 : abs(((mask1 >> F[i]) & 1) + ((mask1 >> F[j]) & 1));
                            ll y = G[i] == G[j] ? 0 : abs(((mask2 >> G[i]) & 1) + ((mask2 >> G[j]) & 1));
                            if(x + y == 0){
                                valid = 0;
                            }
                        }
                ...
                if(valid) ans = min(ans, (ll)(__builtin_popcount(mask1) + __builtin_popcount(mask2)));
            }
        }
        cout << ans << "\n";
    }    
