...
int main(){
    ...
    while (t--){
		...
		ll right[n];
		left[0] = a[0];
		rep_a(i, 1, n) left[i] = left[i-1] ^ a[i];
		right[n-1] = a[n-1];
		rev(i, n - 2) right[i] = right[i+1] ^ a[i];
		ll left_cons[n];
		ll right_cons[n];
		left_cons[0] = a[0];
		rep_a(i, 1, n){
			if(a[i] == left_cons[i-1]) left_cons[i] = a[i];
            else left_cons[i] = -1;
		}
		right_cons[n-1] = a[n-1];
		rev(i, n - 2){
			if(a[i] == right_cons[i+1]) right_cons[i] = a[i]; 
            else right_cons[i] = -1;
		}
		ll flag = 0;
		rep_a(i, 1, n){
			if(left[i-1] == right[i] || left[i-1] == right_cons[i] || left_cons[i-1] == right[i]){
				flag = 1;
				break;
			}
		}
		rep_a(i, 1, n){
			rep_a(j, i + 1, n){
				if(left[i-1] == right[j] && left[i-1] != -1){
					ll temp = a[i];
					for(ll k = i+1; k < j; k++) temp ^= a[k];
					if(temp == left[i-1]) flag = 1;
				}
			}
		}
		if(flag) yes;
