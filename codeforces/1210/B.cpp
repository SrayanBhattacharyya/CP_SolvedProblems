...
const ll N = 7005;
ll now[N], a[N], val[N], sum , n;
bool flg[N][N]; bool vis[N];
...
int main(){
    ...
    while (t--){
        ...
        rep_a(i, 1, n + 1){
            cin >> a[i];
            rep_a(j, 1, i){
                flg[i][j] = (a[i] & (((1ll << 60) - 1) ^ a[j])) != 0;
                flg[j][i] = (a[j] & (((1ll << 60) - 1) ^ a[i])) != 0;;
            }
        }
        rep_a(i, 1, n + 1) {cin >> val[i]; sum += val[i];}
        ll cur = 0; queue < ll > q;
        rep_a(i, 1, n + 1){
            for(ll j = 1 ; j <= n ; ++j) now[i] += flg[i][j];
            if(now[i] == n - 1){vis[i] = 1; q.push(i);}
        }
        while(!q.empty()){
            ll t = q.front(); q.pop(); sum -= val[t]; ++cur;
            rep_a(i, 1, n + 1){
