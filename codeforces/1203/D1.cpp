...
int main(){
    ...
    vll v1, v2;
    vll chars[26];
    rep(i, s.length()) chars[s[i] - 'a'].push_back(i);
    for (ll i = 0, j = 0; i < t.length(); j++) {
        if (s[j] == t[i]) {
            v1.emplace_back(j);
            i++;
        }
    }
    v2.emplace_back(s.length());
    ll ans = s.length() - v1[v1.size() - 1] - 1;
    rev(i, t.length() - 1){
        ll idx = lower_bound(all(chars[t[i] - 'a']), v2[v2.size() - 1]) - chars[t[i] - 'a'].begin();
        v2.emplace_back(chars[t[i] - 'a'][idx - 1]);
        if (i != 0)  ans = max(ans, v2[v2.size() - 1] - v1[i - 1] - 1);
		else ans = max(ans, v2[v2.size() - 1] );
    }
    cout << ans;
    return 0;
}