...
int main(){
    ...
    while (t--){
    	...
    	rep(i, n) cin >> a[i];
    	vll v[k];
    	rep(i, n) v[i % k].emplace_back(i);
    	ll flag = 0;
    	rep(i, k){
    		set<char> abc;
    		for(auto it : v[i]) if(a[it] != '?') abc.insert(a[it]);
    		if(abc.size() == 2)	flag = 1;
    		if(abc.size() == 1)	for(auto it : v[i])	a[it] = *(abc.begin());
    	}
    	ll one = 0, zero = 0, mark = 0;
    	rep(i, k){
    		if(a[i] == '?')	mark++;
    		if(a[i] == '1') one++;
    		if(a[i] == '0')	zero++;
    	}	
    	if(mark < abs(one-zero)) flag = 1;
    	rep_a(i, k, n){
    		if(a[i] == '?')	mark++;
    		if(a[i] == '1')	one++;
    		if(a[i] == '0')	zero++;
