...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
        vll cnt(n + 1, 0), a(n + 1,0);
        vvll adj(n + 1);
        vvll q(n - 1, vll(4, 0));
        rep_a(i, 1, n - 1){
            rep_a(j, 1, 4){
                cin >> q[i][j];
                adj[q[i][j]].push_back(i);
                cnt[q[i][j]]++;
            }
        }
        rep_a(i, 1, n + 1){
            if(cnt[i] == 1){
                a[1] = i;
                rep_a(j, 1, 4){
                    if(cnt[q[adj[i][0]][j]] == 2) a[2] = q[adj[i][0]][j];
                    else if(cnt[q[adj[i][0]][j]] == 3) a[3] = q[adj[i][0]][j];
                }
                break;
            }
        }
        rev_a(i, 1, n){
            if(cnt[i] == 1){
                a[n] = i;
                rep_a(j, 1, 4){
                    if(cnt[q[adj[i][0]][j]] == 2) a[n-1] = q[adj[i][0]][j];
                    else if(cnt[q[adj[i][0]][j]] == 3) a[n-2] = q[adj[i][0]][j];
                }
                break;
            }
        }
        rep_a(i, 4, n - 2){
            for(auto x : adj[a[i-1]]){
                bool f1, f2, f3;
                f1 = f2 = f3 = false;
                rep_a(j, 1, 4){
                    if(q[x][j] == a[i - 3]) f1 = true;
                    if(q[x][j] == a[i - 2]) f2 = true;
                    if(q[x][j] == a[i - 1]) f3 = true;
                }
                if((f3) && (f2) && (!f1)){
                    rep_a(j, 1, 4){
                        if(q[x][j] != a[i-1] and q[x][j] != a[i-2]){
                            a[i] = q[x][j];
                            break;
                        }
                    }
