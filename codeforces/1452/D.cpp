...
ll mods_div(ll a, ll b) {a = a % MODS; ll inv = mods_inv(b); if (inv == -1) return -1; else {ll ans = (inv * a) % MODS; return ans;}}
...
ll modInverse(ll a, ll m)   //m and a should be coprime.
{
    ll m0 = m;
    ll y = 0, x = 1;
    if (m == 1)
...
int main()  {
    ...
    pow[2] = 4;
    for(int i = 3; i <= n; i++)	{
    	ans[i] = (ans[i-2]+ans[i-1]) % MODS;
    	pow[i] = (pow[i-1]*2) % MODS;
    }
    ll g = gcd(ans[n], pow[n]);
    // cout << ans[n] << " " << pow[n] << "\n";
    ans[n] /= g;
    pow[n] /= g;
    
    cout << modDivide(ans[n], pow[n], MODS) << "\n";
    return 0;
}