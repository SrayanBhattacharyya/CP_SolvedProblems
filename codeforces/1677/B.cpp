...
int main(){
    ...
    while (t--){
        ll n, m; cin >> n >> m;
        string s; cin >> s;
        rep(i, n * m) c[i + 1] = s[i] - '0';
        rep_a(i, 1, n * m  + 1) {sum[i] = sum[i - 1] + c[i], f[i] = g[i] = 0;}
        rep_a(i, 1, n * m  + 1){
            if(i >= m) f[i] = f[i - m];
            if(sum[i] - sum[max(0ll, i - m)]) f[i]++;
        }
        rep_a(i, 1, n * m  + 1){
            if(i >= m) g[i] = g[i - m];
			g[i] = g[i] || c[i];
        }
        rep_a(i, 1, n * m  + 1) g[i] += g[i - 1];
        rep_a(i, 1, n * m  + 1) cout << f[i] + g[i] - g[max(0ll, i - m)] << ' ' ; cout << '\n';
    }    
    return 0;
}