...
ll n;
string str;
string revr(string a){
    string res = "";
    for(ll i = 0;i < a.size();i++)res += a[a.size() - i - 1];
    return res;
}
...
int main(){
    ...
    for(ll i = 0;i < str.size();i++){
        string a0 = ml + str[i];
        string a1= str[i] + revr(mr);
        string b0 = mr + str[i];
        string b1 = str[i] + revr(ml);
        if(a0 <= a1)ml = a0;
        else{
            ml = a1;
            swp[0][i] = true;
        }
        if(revr(b0) <= revr(b1))mr = b0;
        else{
            mr = b1;
            swp[1][i] = true;
        }
