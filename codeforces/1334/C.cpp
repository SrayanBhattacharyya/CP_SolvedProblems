...
int main(){
    ...
    while (t--){
        ...
        vll a(n), b(n);
        rep(i, n) cin >> a[i] >> b[i];
        vll pref(n), suff(n), diff(n);
        diff[0] = max(a[0] - b[n - 1], 0ll);
        rep_a(i, 1, n) diff[i] = max(a[i] - b[i - 1], 0ll);
        pref[0] = diff[0];
        rep_a(i, 1, n) pref[i] = pref[i - 1] + diff[i];
        suff[n - 1] = diff[n - 1];
        rev(i, n - 2) suff[i] = suff[i + 1] + diff[i];
        ll mn = LLMAX;
        rep(i, n){
            ll cnt = 0;
            if (i != 0) cnt += pref[i - 1];
            if (i != n - 1) cnt += suff[i + 1];
            cnt += a[i];
            mn = min(mn, cnt);
        }
        cout << mn << "\n";
