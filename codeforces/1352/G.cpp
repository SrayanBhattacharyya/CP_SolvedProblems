...
int main(){
    ...
    while (t--){
        ...
        vll ans;
        rep(i, (n-4)/4) {
            ans.emplace_back(i * 4 + 2); 
            ans.emplace_back(i * 4 + 4); 
            ans.emplace_back(i * 4 + 1); 
            ans.emplace_back(i * 4 + 3);
        }
        ll s = (n-4)/4; s *= 4;
        vvll ends;
        ends.push_back({2, 4, 1, 3});
        ends.push_back({2, 4, 1, 3, 5});
        ends.push_back({1, 4, 2, 6, 3, 5});
        ends.push_back({1, 5, 3, 6, 2, 4, 7});
        vll use = ends[n % 4];
        rep(i, use.size())  ans.emplace_back(s + use[i]);
        print(ans);
    }    
    return 0;
}