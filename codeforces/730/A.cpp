...
bool all_eq(ll n) {
	rep(i, n) if(t[0] != t[i]) return false;
	return true;
}
...
int main(){
    ...
    while(!all_eq(n)){
        ll big = 0;
        rep(i, n) big = max(big, t[i]);
        ll cnt = 0;
        rep(i, n) cnt += (big == t[i]);
        if (cnt == 3){
            vll row;
            rep(i, n) if(big == t[i]) row.emplace_back(i);
            w.push_back(row);
        }
        else{
            vll row;
            ...
            big = 0;
            rep(i, n) if(i != row[0]) big = max(big, t[i]);
            rep(i, n) if(i != row[0] && t[i] == big){
                row.push_back(i);
				break;
            }
            w.push_back(row);
        }
        for(ll i : w.back()) t[i] = max(0LL, t[i] - 1);
    }
    cout << t[0] << "\n";
    cout << w.size() << "\n";
    for(auto row : w){
        for(ll i : row) ta[i] = true;
		rep(i, n) cout << ta[i];
        cout << "\n";
		for(ll i : row) ta[i] = false;
    }
    return 0;
}