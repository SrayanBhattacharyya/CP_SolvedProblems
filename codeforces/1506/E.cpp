...
void placeLeft(vll &q, bool minimize) {
	set<ll> left;
	for (ll i = 1; i <= (ll) q.size(); i++) {
		left.insert(i);
	}
	for (ll i : q) if (i != -1) left.erase(i);
	ll lastPlaced = -1;
	for (ll &i : q) {
		if (i == -1) {
			set<ll> ::const_iterator it;
			if (minimize) it = left.begin(); 
            else it = --left.lower_bound(lastPlaced);
			i = *it;
			left.erase(it);
		} 
        else lastPlaced = i;
...
void solve() {
	...
	cin >> n; vll q(n);
	rep(i, n) cin >> q[i];
    vll res1(n, -1), res2(n, -1);
	for (ll i = 0, lastPlaced = -1; i < n; lastPlaced = q[i], i++) {
		if (lastPlaced != q[i]) {
			res1[i] = q[i];
			res2[i] = q[i];
		}
