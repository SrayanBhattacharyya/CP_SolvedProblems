...
int main(){
    ...
    while (t--){
        ...
        ll pfx = 1 + (n + m - 2);
        if (pfx & 1) {no; continue;}
        vvll sumr(n + 1, vll(m + 1, 0));
        vvll sumc(n + 1, vll(m + 1, 0));
        rep_a(i, 1, n + 1) rep_a(j, 1, m + 1){
            ll x = a[i - 1][j - 1];
            if (x == 1){
                sumr[i][j] = max(sumr[i - 1][j], sumr[i][j - 1]) + 1;
                sumc[i][j] = max(sumc[i - 1][j], sumc[i][j - 1]);
            }
            else{
                sumr[i][j] = max(sumr[i - 1][j], sumr[i][j - 1]);
                sumc[i][j] = max(sumc[i - 1][j], sumc[i][j - 1]) + 1;
            }
        }
        ll ans1 = pfx / 2;
        ll ans2 = sumr[n][m], min1 = pfx - (sumc[n][m]);
        cout << ((min1 <= ans1 && ans1 <= ans2) ? "YES" : "NO") << "\n";
    }
    return 0;
}