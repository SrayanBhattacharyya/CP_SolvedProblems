...
int main(){
    ...
    while (t--){
        ...
        rep(i, n){
            cin >> array[i];
            totalDays += array[i];
            if(i == 0) daysPrefix[i] = array[i];
            else  daysPrefix[i] = array[i] + daysPrefix[i-1];
        }
        ll sum[n];
        ll prefixSum[n];
        rep(i, n){
            sum[i] = (array[i]*(array[i] + 1))/2;
            if(i == 0) prefixSum[i] = sum[i];
            else prefixSum[i] = sum[i] + prefixSum[i-1];
        }
        ll value = x/totalDays;
        x %= totalDays;
        ll finalAns  =0;
        int flag = 0;
        ll index = 0;
        ...
        rep(i, n){
            if(daysPrefix[i] >=  x) {
                ll ans = prefixSum[i];
                ll left = daysPrefix[i] - x;
                while(daysPrefix[index] < left) index++;
                if(index){
                    ans -= prefixSum[index-1];
                    left -= daysPrefix[index-1];
                }
                ans -= (left * (left + 1)) / 2;
                ...
                ll ans = prefixSum[i];
                ll tempX = x - daysPrefix[i];
                ll tempI = n-1;
                ans += prefixSum[tempI];
                ll left = daysPrefix[tempI] - tempX;
                while(daysPrefix[index1] < left) index1++;
                if(index1)   {
                    ans -= prefixSum[index1-1];
                    left -= daysPrefix[index1-1];
                }
                ans -= (left * (left + 1)) / 2;
                finalAns = max(finalAns, ans);
            }
        }
        cout << finalAns + (value * prefixSum[n - 1])<< "\n";
    }    
    return 0;
}