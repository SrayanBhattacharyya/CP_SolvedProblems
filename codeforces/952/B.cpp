...
int main(){
    ...
    while (t--){
		...
		if(s[i] == string("great") ||
			s[i] == string("cool") ||
			s[i] == string("not bad") ||
			s[i] == string("don't touch me") ||
			s[i] == string("don't think so") ){
			flag = 1;
			break;
		}
		else if(s[i] == string("terrible") ||
			s[i] == string("worse") ||
			s[i] == string("are you serious") ||
			s[i] == string("go die in a hole") ||
			s[i] == string("don't even") ||
			s[i] == string("no way")){
			flag = 2;
			break;
		}
