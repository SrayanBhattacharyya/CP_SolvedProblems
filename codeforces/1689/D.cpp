...
int main(){
    ...
    while (t--){
        ...
        rep(i, n){
            cin >> a[i];
            rep(j, m){
                if(a[i][j] == 'B') black.push_back({i + 1,j + 1});
            }
        }
        ll mxplus = -LLMAX, mxdiff = -LLMAX, mnplus = LLMAX,mndiff = LLMAX;
        for (auto x : black){
            mxplus = max(mxplus, x.first + x.second);
            mxdiff = max(mxdiff, x.first - x.second);
            mnplus = min(mnplus, x.first + x.second);
            mndiff = min(mndiff, x.first - x.second);
        }
        ll r = -1, c = -1;
        ll ans = LLMAX;
        rep_a(i, 1, n + 1){
            rep_a(j, 1, m + 1){
                ll mx = -LLMAX;
                mx = max(mx, i + j - mnplus);
                mx = max(mx, j - i + mxdiff);
                mx = max(mx, i - j - mndiff);
                mx = max(mx,mxplus - i - j);
                if(mx < ans){
                    ans = mx;
                    r = i;
                    c = j;
