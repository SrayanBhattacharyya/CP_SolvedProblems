...
int main(){
    ...
    while (t--){
		...
		if (!is_sorted(a.begin() + k, a.end() - k)) {
			cout << "NO\n";
			continue;
		}
		vll c;
		rep(i, k) c.push_back(a[i]);
		for (ll i = n - k; i < n; ++i) c.push_back(a[i]);
		sort(all(c));
		vll l{c.begin(), c.begin() + k};
		vll r{c.begin() + k, c.end()};
		bool flag = true;
		ll low = *l.rbegin();
		ll high = *r.begin();
		for (ll i = k; i < n - k; ++i) if (a[i] < low || a[i] > high) flag = false;
		cout << (flag ? "YES\n" : "NO\n"); 
    }    
    return 0;
}