...
int main(){
    ...
    while (t--){
        ...
        rep(i, n) cin >> a[i];
        rep(i, n) cin >> b[i];
        ll firA = LLMAX, secA = LLMAX;
        ll firB = LLMAX, secB = LLMAX;
        rep(i, n){
            firA = min(firA , abs(a[0] - b[i]));
            firB = min(firB, abs(b[0] - a[i]));
            secA = min(secA, abs(a[n - 1] - b[i]));
            secB = min(secB, abs(b[n - 1] - a[i]));
        }
        ll ans = firB + firA + secA + secB;
        ans = min(ans, firA + firB + abs(b[n - 1] - a[n - 1]));
        ans = min(ans, secA + firB + abs(b[n - 1] - a[0]));
        ans = min(ans, secA + secB + abs(b[0] - a[0]));
        ans = min(ans, firA + secB + abs(b[0] - a[n - 1]));
        ans = min(ans, abs(b[0] - a[0]) + abs(b[n - 1] - a[n - 1]));
        ans = min(ans, abs(b[0] - a[n - 1]) + abs(b[n - 1] - a[0]));
        cout << ans << "\n";
    }    
    return 0;
}