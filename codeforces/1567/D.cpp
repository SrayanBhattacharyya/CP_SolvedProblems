...
int main(){
    ...
    cout.tie(0);
    pw10[0] = 1;
    rep_a(i, 1, 10) pw10[i] = pw10[i-1] * 10;
    ll t = 1;
    cin >> t;
    while (t--){
        ll n, m; cin >> n >> m;
        ...
        while (c.size() < m){
            ...
            for (ll x : c){
                if (x > 0 && !ok){
                    rep(j, 10) nc.emplace_back(x - 1);
                    ok = true;
                }
                else nc.emplace_back(x);
            }
            c = nc;
        }
        rep(i, m - 1) cout << pw10[c[i]] << " ";
        ll res = 0;
        for (ll i = m - 1; i < c.size(); i++) res += pw10[c[i]];
        cout << res << "\n";
    }    
    return 0;
}