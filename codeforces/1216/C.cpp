...
ll x1, yy1, x2, y2, x3, x4, y3, y4, x5, y5, x6, y6;

bool inside(ll x, ll y){
    if(x >= x3 && x <= x4 && y >= y3 && y <= y4) return 0;
    if(x >= x5 && x <= x6 && y >= y5 && y <= y6) return 0;
    return 1;
}

bool check(){
    rep_a(i, x1, x2) if(inside(i, yy1) || inside(i, y2)) return 1;
    rep_a(i, yy1, y2) if(inside(x1, i) || inside(x2, i)) return 1;
    return 0;
}
...
int main(){
    ...
    while (t--){
        ...
        cin >> x3 >> y3 >> x4 >> y4;
        cin >> x5 >> y5 >> x6 >> y6;
        x1 <<= 1; yy1 <<= 1; x2 <<= 1; y2 <<= 1; x3 <<= 1; y3 <<= 1;
        x4 <<= 1; y4 <<= 1; x5 <<= 1; y5 <<= 1; x6 <<= 1; y6 <<= 1;
        if (check()) yes;
        else no;
    }    
    return 0;
