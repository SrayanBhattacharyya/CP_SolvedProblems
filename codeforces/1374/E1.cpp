...
int main(){
    ...
    while (t--){
        ...
        rep(i, n){
            ll x, a, b;
            cin >> x >> a >> b;
            if(a==1 && b==1) com.emplace_back(x);
            else if(a==1 && b==0) ali.emplace_back(x);
            else if(a==0 && b==1) bob.emplace_back(x);
        }
        ll mn = min(ali.size(), bob.size());
        st(ali); st(bob);
        rep(i, mn) com.emplace_back(ali[i] + bob[i]);
        if (com.size() < k) cout << -1 << "\n";
        else{
            st(com);
            ll sum = 0;
            rep(i, k) sum += com[i];
