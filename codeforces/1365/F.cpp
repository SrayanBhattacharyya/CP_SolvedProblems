...
void swapping(ll index, ll current, ll flag) {
    ll a1[index + 1];
    rep_a(i, 1, index + 1)  {
        a1[i] = a[i];
        a[i] = a[n - index + i];
    }
    rep_a(i, 1, index + 1) a[n - i + 1] = a1[index - i + 1];
    if(flag == 1) swap(a[1], a[n]);
    ll a2[current + 1];
    rep_a(i, 1, current + 1){
        a2[i] = a[i];
        a[i] = a[n - current + i];
    }
    rep_a(i, 1, current + 1) a[n - i + 1] = a2[current -i + 1];
}
...
int main(){
    ...
    while (t--){
        ...
        while(current > 0){
            ...
            flag = 0;
            rep_a(i, 1, current + 1){
                if(a[i] == abc1 && a[n-i+1] == abc2) {
                    swapping(i, current, 0);
                    flag = 1;
                    break;
                }
            }
            if(flag == 0)   {
                rep_a(i, 1, current + 1){
                    if(a[n - i + 1] == abc1 && a[i] == abc2) {
                        swapping(i, current, 1);
                        flag = 1;
                        break;
                    }
                }
