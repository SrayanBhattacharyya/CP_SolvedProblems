...
void solve(){ 
        ...
        vector <pair <ll,ll>> v;
        v.push_back(mpa((ll)0,(ll)0));
        for(ll i=0;i<=5e3;i++){
                vector <pair<ll,ll>> temp,cur;
                ll j = 0;
                for(auto x : v){
                        while(j<(ll)ind[i].size() && x.first>=ind[i][j]){
                                j++;
                        }
                        if(j>=(ll)ind[i].size()){
                                break;
                        }
                        temp.push_back(mpa(ind[i][j],(x.second^i)));
                }
                vector <bool> vis(m,0);
                ll a = 0,b = 0;
                while(a<(ll)v.size() && b<(ll)temp.size()){
                        if(v[a].first<=temp[b].first){
                                if(!vis[v[a].second]){
                                        vis[v[a].second] = true;
                                        cur.push_back(v[a]);
                                }
                                ...
                                if(!vis[temp[b].second]){
                                        vis[temp[b].second] = true;
                                        cur.push_back(temp[b]);
                                }
                                b++;
                        }
                }
