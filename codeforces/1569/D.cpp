...
ll solve1 (vll v){
    ...
    d.emplace_back(k);
    ll res = 0;
    rep(i, d.size()) res += d[i] * ((ll)v.size() - d[i]);
    return res / 2; 
}

ll solve2 (vpll v, vll d){
    d.push_back(-1);
    d.push_back(1e6 + 1);
    st(d);
    map <ll, vll > mp;
    rep(i, v.size()) mp[v[i].first].push_back(v[i].second);
    ll res = 0;
    rep_a(i, 1, d.size()){
        auto it = mp.upper_bound(d[i-1]);
        vll cur;
        while(it!=mp.end() && (*it).first<d[i]){
            for(auto x : it->second)  cur.push_back(x);
            it++;
        }
        if(!cur.empty()) res += solve1 (cur);
    }
    return res;
}
...
void solve(){
    ...
    ans += solve2(v,vx);
    rep(i, k) swap(v[i].first,v[i].second);
    ans += solve2(v,vy);
    cout << ans << "\n";
}
