...
void solve(){
    ll n; cin >> n;
    vpll steps; multiset <ll> s;
    n <<= 1;
    rep(i, n) cin >> a[i];
    sort(a, a + n);
    rep(i, n - 1){
        s.clear();
        rep(j, n - 1) if (j != i) s.insert(a[j]);
        ll x = a[n - 1];
        steps.clear(); steps.push_back({a[n-1], a[i]});
        while (s.size() > 0) {
            ll p = *s.rbegin();
            s.erase(s.find(p));
            if (!s.count(x - p)) break;
            s.erase(s.find(x - p));
            steps.push_back({p, x-p});
            x = p;
        }
        if (s.size() == 0) {
            yes;
            rep(j, steps.size()) {
                if (steps[j].first > steps[j].second) swap(steps[j].first, steps[j].second);
                if (j==0) cout << steps[j].first + steps[j].second << '\n';
                cout << steps[j].first << ' ' << steps[j].second << '\n';
            }		
            return;
        }
    }
