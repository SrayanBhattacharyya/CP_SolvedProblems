...
int main(){
    ...
    while (t--){
        ...
        ll ans = 0;
        bool start[n + 3] = {false};
        start[0] = true;
        start[n] = true;
        rev(j, 30){
            rep(i, n){
                a[i].first = ((a[i].second) >> j) & 1;
                b[i].first = ((b[i].second) >> j) & 1;
            }
            vll v;
            rep(i, n + 1) if(start[i]) v.emplace_back(i);
            ll len = v.size();
            rep(i, len - 1){
                sort(a.begin() + v[i], a.begin() + v[i + 1] );
                sort(b.begin() + v[i], b.begin() + v[i + 1] );
                reverse(b.begin() + v[i], b.begin() + v[i + 1] );
            }
            bool flag = false;
            rep(i, n){
            ...
            if(flag) continue;
            ans = ans | (1 << j);
            rep_a(i, 1, n) if(a[i].first != a[i - 1].first) start[i] = true;
        }
        cout << ans << "\n";
    }     
    return 0;
