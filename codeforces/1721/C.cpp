...
int main(){
    ...
    while (t--){
        ...
        rep(i, n) cin >> b[i];
        st(b);
        vll ansmin(n), ansmax(n);
        rep(i, n) ansmin[i] = *lower_bound(all(b), a[i]) - a[i];
        set <pll> st;
        rep(i, n) st.insert({b[i], i});
        rev(i, n - 1){
            ansmax[i] = (st.rbegin() -> first) - a[i];
            st.erase(st.lower_bound({a[i], -1}));
        }
        rep(i, n) cout << ansmin[i] << " \n"[i == n - 1];
        rep(i, n) cout << ansmax[i] << " \n"[i == n - 1];
    }    
    return 0;
}