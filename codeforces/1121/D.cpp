...
int main(){
    ...
    while (t--){
        ...
        bool ok = false;
        ll r = -1;
        while(!req.empty() && r + 1 < n){
            r++;    
            curcnt[v[r]]++;
            if(req.find(v[r]) != req.end()) req.erase(req.find(v[r]));
        }
        vll ln(n, LLMAX);
        if(req.empty()){
            ok = true;
            ln[0] = r;
            curcnt[v[0]]--;
            if(rcnt[v[0]]  >curcnt[v[0]]) ok = false;
            rep_a(i, 1, n){
                if (!ok){
                    while(r + 1 < n && !ok){
                        r++;
                        curcnt[v[r]]++;
                        if(curcnt[v[i - 1]] >= rcnt[v[i - 1]]) ok = true;
                    }
                }
                if (!ok) break;
                ln[i] = r;
                curcnt[v[i]]--;
                if(rcnt[v[i]]>curcnt[v[i]]) ok = false;
            }
        }
        ll ans = LLMAX, ansl = -1, ansr;
        req.clear();
        ll l = 0;
        r = -1;
        rep(i, m){
            while(l < i * k){
                req.erase(req.find(ln[l]));
                l++;
            }
            while(r + 1 < n - (m - i - 1) * k){
                r++;
        ...
        if (ansl == -1) cout << -1 << "\n";
            ...
            cout << max(ans, 0ll) << "\n";
            map <ll,ll> mp; vll ansvec;
            ll cur = max(ans, 0ll);
            ansr = min(ansr, ansl + cur + k - 1);
            rep_a(i, ansl, ansr + 1) mp[v[i]]++;
            rep_a(i, ansl, ansr + 1){
                if (cur){
                    if(mp[v[i]] > rcnt[v[i]]){
                        cur--;
                        ansvec.emplace_back(i);
                        mp[v[i]]--;
                    }
                }
            }
            for(auto x : ansvec) cout << x + 1 << " ";
            cout << "\n";
        }
    }    
    return 0;
