...
int main(){
    ...
    ll n, m;
    cin >> n; vll a(n);
    rep(i, n) cin >> a[i]; st(a);
    cin >> m; vll b(m);
    rep(i, m) cin >> b[i]; st(b);
    ll ans  = 2 * n - 2 * m, ma = 2 * n, mb = 2 * m, d, s, t;
    rep(i, n){
        d = a[i];
        ll l = lower_bound(all(a), d) - a.begin();
        s = l * 2;
        s += (n - l) * 3;
        l = lower_bound(all(b), d) - b.begin();
        t = 2 * l;
        t += (m - l) * 3;
        if(s - t > ans){
            ans = s-t;
