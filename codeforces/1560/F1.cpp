...
int main(){
    ...
    while (t--){
		...
		rep(st, n){
			...
			if (all.size() > k or s[st] == '9') continue;
			if (all.size() < k){
				cur = cur * 10 + s[st] - '0' + 1;
				all.insert(s[st] - '0' + 1);
			}
			else {
				auto it = upper_bound(all.begin(), all.end(), s[st] - '0');
				if (it == all.end()) continue;
				cur = cur * 10 + *it;
			}
			rep_a(i, st + 1, n){
				if (all.size() < k) cur = cur * 10;
				else cur = cur * 10 + *all.begin();
			}
			ans = min(ans, cur);
		}
		sll all;
		for (char i : s) all.insert(i - 'a');
		if (all.size() <= k) cout << s << '\n';
		else cout << ans << '\n';
    }    
    return 0;
}