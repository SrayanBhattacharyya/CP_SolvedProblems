...
bool check(string s, int ind) {
	rep(i, 7){
		if(s[ind + i] == '?') s[ind + i] = sr[i]; 
		else if(s[ind + i] != sr[i]) return false;
	}
	ll cnt = 0;
	ll n = s.size();
	rep(i, n) if(s[i] == '?') s[i] = 'z';
	rep(i, n) if(s.substr(i, 7) == sr) cnt++;
	if(cnt == 1) {
		yes;
		cout << s << "\n";
		return true;
...
int main(){
    ...
    while (t--){
        cin >> n;
        _clear();
        rep_a(i, 1, n + 1) {cin >> a[i]; freq[a[i]]++;}
        cout << freq[0] << ' ';
        rep_a(i, 1, freq[0] + 1) s.insert(0);
        rep_a(i, 1, n + 1){
            if( q ) {cout << -1 << " "; continue;}
            if( s.empty()){
                q = 1;
                {cout << -1 << " "; continue;}
            }
