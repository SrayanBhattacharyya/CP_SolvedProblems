...
int main(){
    ...
    while (t--){
        ...
        vpll a, b; rep(i, n){
            cin >> k; s1 += k;
            if (i != 0 && a[a.size() - 1].first == k) a[a.size() - 1].second++;
            else if (k % m == 0){
                ll x = 1, c;
                while (k % m == 0) {x *= m; c = k / m; k /= m;}
                if (i != 0 && a[a.size() - 1].first == c) a[a.size() - 1].second += x;
                else a.push_back({c, x});
            }
        ...
        rep(i, n){
            cin >> k; s2 += k;
            if (i != 0 && b[b.size() - 1].first == k) b[b.size() - 1].second++;
            else if (k % m == 0){
                ll x = 1, c;
                while (k % m == 0) {x *= m; c = k / m; k /= m;}
                if (i != 0 && b[b.size() - 1].first == c) b[b.size() - 1].second += x;
                else b.push_back({c, x});
            }
            else b.push_back({k, 1});
        }
        bool fl = true;
        if ((s1 != s2) || (b.size() != a.size())) fl = false;
        else rep(i, a.size()) if (a[i].first != b[i].first || a[i].second != b[i].second) fl = false;
        cout << (fl? "YES" : "NO") << "\n";
    }    
    return 0;
}