...
void solve(){
    ll n, m; cin >> n >> m;
    vector <multiset <ll>> red(m), blue(m);
    vvll a(n, vll(m));
    rep(i, n) rep(j, m) cin >> a[i][j], red[j].insert(a[i][j]);
    vpll color;
    rep(i, n) color.push_back({a[i][0], i});
    sort(all(color));
    ...
    rep(i, n){
		...
		isBlue[ID] = 1;
		rep(j, m){
			red[j].erase(red[j].lower_bound(a[ID][j]));
			blue[j].insert(a[ID][j]);
		}
		cnt++;
		while(i + 1 < n && color[i + 1].first == vv){
			i++;
			auto [vv1, id] = color[i];
			isBlue[id] = 1;
			cnt++;
		...
		ll mx = 0;
		ll miRed = LLMAX, mxBlue = 0, mxRed = 0, miBlue = LLMAX;
		rep(j, m-1){
		...
		rep_a(j, mx+1, m) ckmax(mxRed, *--red[j].end()), ckmin(miBlue, *blue[j].begin());
		if(mxRed < miBlue){
			yes;
			rep(i, n) cout << (isBlue[i] ? 'B':'R');
			cout << " " << mx + 1 << "\n";
			return;
		}
    }
