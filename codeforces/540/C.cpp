...
int main(){
    ...
    while (t--){
        ...
        while (!q.empty()){
            cur = q.front();
            q.pop();
            if (cur.first < 0 || cur.second < 0 || cur.first >= n || cur.second >= m) continue;
		    if (cur == cur2 && a[cur.first][cur.second] == 'X'){yes; return 0;}
            if (a[cur.first][cur.second] == 'X') continue;
            a[cur.first][cur.second] = 'X';
            pll cur3 = cur;
            cur3.first++; q.push(cur3);
            cur3.first -= 2; q.push(cur3);
            cur3.first++; cur3.second++; q.push(cur3);
            cur3.second -= 2; q.push(cur3);
        }
        no;
    }    
    return 0;
