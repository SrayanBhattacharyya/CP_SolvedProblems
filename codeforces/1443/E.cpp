...
int main() {
  ...
  while (q--) {
    ...
    if (t == 1) {
      vll v, a;
      for (ll i = min(15LL, n) - 1; i >= 0; i--) {
        a.emplace_back(n - i);
      }
      for (ll i = min(15LL, n); i >= 1; i--) {
        ll idx = x % fact[i] / fact[i - 1];
      ...
      ll ans = 0;
      if (l <= n - 15) {
        for (ll i = 0; i + n - min(15LL, n) + 1 <= r; i++) {
          ans += v[i];
        }
        ans += min(r, (ll) n - 15) * (min(r, (ll) n - 15) + 1) / 2 - l * (l - 1) / 2;
      } else {
        l--; r--;
        for (ll i = l - n + min(15LL, n); i <= r - n + min(15LL, n); i++) {
          ans += v[i];
        }
      }
      cout << ans << '\n';
