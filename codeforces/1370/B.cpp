...
int main(){
    ...
    while (t--){
        ...
        while(res.size() != n - 1 && cnt[0].size() >= 2){
            ll x=cnt[0].back(); cnt[0].pop_back();
			ll y=cnt[0].back(); cnt[0].pop_back();
			res.emplace_back(x,y);
        }
        while(res.size() != n - 1 && cnt[1].size() >= 2){
            ll x=cnt[1].back(); cnt[1].pop_back();
			ll y=cnt[1].back(); cnt[1].pop_back();
			res.emplace_back(x,y);
        }
