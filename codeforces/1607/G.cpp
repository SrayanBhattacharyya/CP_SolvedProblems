...
int main(){
    ...
    while (t--){
        ...
        rep(i, n){
            ...
            ans[i].first = min(m, v[i].first);
            ans[i].second = max(0ll , m - v[i].first);
            q.push({min(v[i].second,m) - ans[i].second, i});
            cur += ((v[i].first - ans[i].first) - (v[i].second - ans[i].second));
        }
        while (true){
            auto u = q.top();
            q.pop();
            ll i = u.second;
            if(u.first == 0 || cur >= 0) break;
            else{
                if (2 * u.first + cur >= 0){
                    ll temp = cur;
                    cur -= (v[i].first - ans[i].first - (v[i].second - ans[i].second));
