...
int main(){
    ...
    while (t--){
        ll n, cnt = 0; cin >> n;
        vll a(n); bool fl = false;
        map <ll, ll> mp; vll a1;
        rep(i, n){
            cin >> a[i];
            if (a[i] != 0) fl = true;
            else cnt++; mp[a[i]]++;
            if (mp[a[i]] <= 3) a1.emplace_back(a[i]);
        }
        if(fl == false) {yes; continue;}
        st(a1);
        if (cnt == n - 2 && a1[0] == -1 * a1[a1.size() - 1])  {yes; continue;}
        if (a1.size() > 10) no;
        else{
            ll s = a1.size();
            bool fl1 = true;
            rep(i, s) rep_a(j, i + 1, s) rep_a(k, j + 1, s) if(mp.count(a1[i] + a1[j] + a1[k]) == 0) fl1 = false;
            cout << (fl1 ? "YES" : "NO") << "\n";
        } 
    }    
    return 0;
