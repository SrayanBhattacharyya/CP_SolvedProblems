...
int main(){
    ...
    while (t--){
    	...
    	cin >> n;
    	vll ind[n + 1];
    	rep_a(i, 1, n + 1) ind[i].emplace_back(-1);
    	vll a(n);
    	rep(i, n){
    		cin >> a[i];
    		ind[a[i]].emplace_back(i);
    	}
    	rep_a(i, 1, n + 1) ind[i].emplace_back(n);
    	vll diff[n + 1]; vll ans(n + 1, 0);
    	ll maxi = n + 1;
    	rep_a(i, 1, n + 1){
    		for(ll j = 1; j < ind[i].size(); j++)	{
    			diff[i].emplace_back(ind[i][j] - ind[i][j-1]);
    		}
    		st(diff[i]);
    		if(diff[i].back() < maxi){
    			ans[diff[i].back()] = i;
