   ...
   ll t;
    ...
    while(t--)	{
    	...
    	rep(i, 19){
    		...
    		ll curY = 0;
    		rep(j, 19){
    			while(ind != n && (v[ind] + curY) <= y)	{
    				curY += v[ind];
    				ind++;
    			}
    			ll ansY = y - curY;
    			ll rest = total - curX - curY;
    			ll ansZ = *lower_bound(power2.begin(), power2.end(), rest) - rest;
    			ans = min(ans, ansX + ansY + ansZ);
    			y *= 2;
    		}
    		x *= 2;
    	}
