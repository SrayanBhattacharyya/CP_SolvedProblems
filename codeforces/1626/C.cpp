...
int main(){
    ...
    while (t--){
        ...
        mi[n - 1] = v[n - 1].first - v[n - 1].second + 1;
        rev(i, n - 2) mi[i] = min(mi[i + 1], v[i].first - v[i].second + 1);
        rep(i, n - 1) if (mi[i] < mi[i + 1]) if (mi[i + 1] <= v[i].first) mi[i + 1] = mi[i];
        ll ans = 0;
        rep(i, n) if (i == n - 1 || mi[i] < mi[i + 1]) ans += (v[i].first - mi[i] + 2) * (v[i].first - mi[i] + 1) / 2;
        cout << ans << "\n";
    }    
    return 0;
}