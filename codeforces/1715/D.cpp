...
int main(){
    ...
    while (q--){
        ...
        --i; --j;
        if (i == j){
			rep(k, 30) ans[i][k] = (x >> k & 1);
			continue;
		}
        if (i > j) swap(i, j);
        rep(k, 30){
            if (x >> k & 1) v[k][i].emplace_back(j);
			else ans[i][k] = ans[j][k] = 0;
        }
    }
    rep(k, 30) rep(i, n){
        st(v[k][i]);
        for(auto x : v[k][i]) if (ans[x][k] == 0){
            ans[i][k]=1;
            break;
        }
        if (ans[i][k] != 1) for(auto x : v[k][i]) ans[x][k] = 1;
    }
    rep(i, n){
        rep(k, 30) if (ans[i][k] == 1) ans2[i] |= (1 << k);
        cout << ans2[i] << " ";
    }
    cout << "\n";
    return 0;
