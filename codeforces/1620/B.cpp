...
int main(){
    ...
    while (t--){
        ll w, h; cin >> w >> h;
        ll k; cin >> k;
        vll x0(k); rep(i, k) cin >> x0[i];
        cin >> k;
        vll xh(k); rep(i, k) cin >> xh[i];
        cin >> k;
        vll y0(k); rep(i, k) cin >> y0[i];
        cin >> k;
        vll yw(k); rep(i, k) cin >> yw[i];
        ll ans = 0;
        if(x0.size() != 0 && x0.size() != 0){
            sort(all(x0)); sort(all(xh));
            ans = max(ans, (x0.back() - x0[0]) * h);
            ans = max(ans, (xh.back() - xh[0]) * h);
        }
        if(y0.size() != 0 && yw.size() != 0){
            st(y0); st(yw);
            ans = max(ans, (y0.back() - y0[0]) * w);
            ans = max(ans, (yw.back() - yw[0]) * w);
        }
        cout << ans << "\n";
    }    
    return 0;
