...
int main(){
    ...
    while (t--){
        ...
        vpll edges[1005];
        rep(i, v.size()) edges[v[i].first].push_back({a[v[i].second], v[i].second});
        vpll ans;
        for(ll i = 1004; i >= 0; i--)	{
            sort(all(edges[i]));
            for(ll j = 0; j < (ll)edges[i].size(); j++) ans.push_back({edges[i][j].second, i});
        }
        rep(i, ans.size()) swap(a[ans[i].first], a[ans[i].second]);
        ll flag = 0;
        rep_a(i, 2, n + 1){
            if(a[i] < a[i - 1]) flag = 1;
        }
