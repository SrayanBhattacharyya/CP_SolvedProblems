...
int main(){
    ...
    while (t--){
        ll n; cin >> n;
		vector<ll> t(n+1), want(n+1), where(n+1);
		...
		{
			// trace(i);
			ll dis = abs(curr - want[i]);
			where[i] = curr;
 
			ll j = i+1;
			while(j < n+1 and dis > t[j] - t[j-1])
			{
				ll here = t[j] - t[j-1];
				curr = (curr >= want[i] ? curr - here : curr + here);
				where[j] = curr;
				dis -= here;
				j++;
			}
		...
		for(ll i=0; i < n ; ++i)
		{
			ll L = min(where[i], where[i+1]);
			ll R = max(where[i], where[i+1]);
			if(L <= want[i] and want[i] <= R)
				ans++;
		}
