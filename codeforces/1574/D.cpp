...
void solve(){
        ...
        vector <pair<ll, vll>> cur;
        rep(j, v[0].size()) cur.push_back(make_pair(v[0][j], vll {j}));
        rep_a(i, 1, n){
            ll l = 0, r = LLMAX;
            while(l <= r){
                ll mid = (l + r) / 2;
                ll sz = 0;
                rep(j, cur.size()){
                    auto it = upper_bound(all(v[i]), mid - cur[j].first);
                    sz += (v[i].end() - it);
                }
                if(sz > min(m + 50, (ll)cur.size() * (ll)v[i].size()) - 1) l = mid + 1;
                else  r = mid - 1;
            }
            vector <pair<ll,vector <ll>>> t;
            for(auto x : cur){
                ll b = (ll)v[i].size() - 1;
                while(b >= 0 && x.first + v[i][b] > r){
                    vector <ll> temp = x.second;
                    temp.push_back(b);
                    t.push_back(make_pair(x.first + v[i][b], temp));
                    b--;
                }
            }
            sort(all(t));
