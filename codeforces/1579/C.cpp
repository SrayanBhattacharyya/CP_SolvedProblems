...
int main(){
    ...
    while (t--){
        ll n, m, k; cin >> n >> m >> k;
		vector<vector<bool>> mat(n, vector<bool>(m)), ans = mat;
		rep(i, n){
			rep(j, m) {
				char c; cin >> c;
				mat[i][j] = c == '*';
		...
		rep(i, n){
			rep(j, m) if (mat[i][j]) {
				...
				for (ll r = i-1; r >= 0; r--) {
					ll dist = i-r;
					if (j-dist >= 0 and j+dist < m and mat[r][j-dist] and mat[r][j+dist]) len++;
					else break;
				}
				if (len >= k) {
					for (ll r = i; r >= i-len; r--) {
						ll dist = i-r;
						ans[r][j - dist] = ans[r][j + dist] = true;
					}
				}
			}
		}
