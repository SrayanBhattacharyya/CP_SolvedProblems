...
int main(){
    ...
    //cin >> t;
    while (t--){
        ll n, x[1200], y[1200], r[1200]; cin >> n;
        if (n == 1) {cin >> x[1] >> y[1]; cout << "t" << "\n"; cout << y[1] << "\n";}
        else{
            rep_a(i, 1, n + 1) cin >> x[i] >> y[i] >> r[i];

            rep_a(i, 1, n) cout << '(';

            cout << '(' << (x[1]+1)/2 << "*((1-abs((t-1)))+abs((abs((t-1))-1))))+";

            rep_a(i, 2, n) cout << "("<<(x[i]+1)/2 << "*((1-abs((t-" << i << ")))+abs((abs((t-" << i << "))-1)))))+";

            cout<<'('<<(x[n]+1)/2 << "*((1-abs((t-" << n << ")))+abs((abs((t-" << n << "))-1)))))\n";

            rep_a(i, 1, n) cout<<'(';

            cout<<'('<<(y[1]+1)/2<<"*((1-abs((t-1)))+abs((abs((t-1))-1))))+";

            rep_a(i, 2, n) cout<<"("<<(y[i]+1)/2<<"*((1-abs((t-"<<i<<")))+abs((abs((t-"<<i<<"))-1)))))+";

            cout<<'('<<(y[n]+1)/2<<"*((1-abs((t-"<<n<<")))+abs((abs((t-"<<n<<"))-1)))))\n";
        }
    }    
    return 0;
}