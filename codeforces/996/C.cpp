...
ll rotate(){
        ...
        pll start;
        for(ll i=0;i<m;i++){
                if(v[1][i] != 0 && v[1][i] == v[0][i]){
                        ans.push_back(vll {v[1][i],1,i+1});
                        v[1][i] = 0;
                }
                if(v[2][i] != 0 && v[2][i] == v[3][i]){
                        ans.push_back(vll {v[2][i],4,i+1});
                        v[2][i] = 0;
                }
                if(v[1][i] || v[2][i]) fl1 = false;
                if((!v[1][i]) || (!v[2][i])) fl2 = false;
                if(!v[1][i])  start = make_pair(1,i);
                if(!v[2][i])  start = make_pair(2,i);
        }
        if(fl1) return 2;
        if(fl2) return 0;
        pll pre,cur;
        ...
        while(true){
                ...
                if(v[cur.first][cur.second]) ans.push_back(vll {v[cur.first][cur.second],pre.first+1,pre.second+1});
                v[pre.first][pre.second] = v[cur.first][cur.second];
